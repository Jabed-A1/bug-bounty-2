...
Controlled and safe port scanning using nmap
"""
import subprocess
...
class PortScanner:
    ...
    Only scans alive hosts with rate limiting
    """
    ...
    def __init__(self, target, port_range='top1000'):
        ...
        self.timeout = 300  # 5 minutes per host
    
    def scan_all_hosts(self) -> Dict[str, any]:
        """
        Scan all alive hosts for the target
    ...
    def _scan_host(self, hostname: str) -> Dict:
        ...
        try:
            ...
            cmd = [
                'nmap',
                '-sS',  # SYN scan (requires root, falls back to connect scan)
                '-Pn',  # Skip host discovery (we know it's alive)
                '--max-rate', str(self.rate_limit),
                '--max-retries', '1',
                '--host-timeout', f'{self.timeout}s',
                '-sV',  # Service version detection (light)
                '--version-light',  # Don't be aggressive
    ...
    def _save_port(self, hostname: str, port_data: Dict) -> bool:
        ...
        try:
            ...
            # Save port for each live host (they share the same subdomain)
    ...
    def get_statistics(target_id: int) -> Dict:
        """Get port scanning statistics for a target"""
