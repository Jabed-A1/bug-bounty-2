...
Discovers URLs and endpoints from multiple sources
"""
import subprocess
import logging
...
class EndpointCollector:
    ...
    Normalizes, deduplicates, and extracts parameters
    """
    
    def __init__(self, target):
        self.target = target
        self.domain = target.domain
        self.endpoints = {}  # url -> endpoint data
        self.js_files = set()
    ...
    def collect_all(self) -> Dict[str, any]:
        ...
        # Process and save all endpoints
    ...
    def _run_gau(self) -> Set[str]:
        ...
        try:
            cmd = [
                'gau',
                '--blacklist', 'png,jpg,jpeg,gif,svg,css,woff,woff2,ttf,eot',  # Skip static assets
                '--threads', '5',
                self.domain
            ]
    ...
    def _parse_endpoint(self, url: str) -> Dict:
        """Parse endpoint and extract metadata"""
        parsed = urlparse(url)
        params = parse_qs(parsed.query)
        
        return {
            'url': url,
            'method': 'GET',  # Default, will be updated if we detect POST
    ...
    def _get_source_for_url(self, url: str, source_sets: List[tuple]) -> str:
        """Determine which tool(s) discovered this URL"""
        sources = []
    ...
    def get_statistics(target_id: int) -> Dict:
        """Get endpoint statistics for a target"""
